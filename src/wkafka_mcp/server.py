...
def get_wkafka_architect_manual() -> str:
    ...
    return (
        ...
        "14. PYDANTIC VALIDATION: Use `format='pydantic'` and pass `model=MyModel` to validate and deserialize typed schemas automatically.\n"
        "15. MANUAL OFFSETS: Set `@kafka.consumer(auto_commit=False)` and invoke `msg.commit()` manually inside handler callbacks.\n"
        "16. RETRIES & DLQ: Set `max_retries=3`, `retry_delay=1.0`, and `dlq_topic='my_topic.DLQ'` for automatic backoff retries and Dead Letter Queue routing.\n"
        "17. ASYNC HANDLERS: Write `async def my_handler(msg):` consumer callbacks; WKafka runs them natively in event loop.\n\n"
        ...
        "NEED binary/document file transfer? -> WKafka() + format='file' (raw file bytes or document contents).\n"
        "NEED Pydantic model validation? -> WKafka() + format='pydantic' + model=MyPydanticClass.\n"
        "NEED true consumer parallelism & partition scaling? -> kafka.run_consumers(partition_scale=True).\n"
        "NEED manual commit control? -> @kafka.consumer(auto_commit=False) + msg.commit().\n"
        "NEED automatic retries & DLQ routing? -> @kafka.consumer(max_retries=3, retry_delay=1.0, dlq_topic='topic.DLQ').\n"
        "NEED async/await handlers? -> @kafka.consumer() + async def handler(msg):\n"
        ...
        "--- WPIPE & WKAFKA INTEGRATION ARCHITECTURE PATTERN ---\n"
        "To build a structured WPipe pipeline worker microservice integrated with WKafka, follow the standard WPipe directory layout:\n"
        "1. DTOs (`dto/`): Define Pydantic BaseModel context objects representing the streaming payloads.\n"
        "2. STATES (`states/`): Wrap WKafka consumer and producer logic into class-based WPipe steps decorated with `@step`.\n"
        "3. PIPELINE (`main.py`): Orchestrate the pipeline execution with `Pipeline(tracking_db='tracking.sqlite')` and `pipeline.set_steps([...])`.\n\n"
        "```python\n"
        "# dto/stream_context.py\n"
        "from pydantic import BaseModel\n"
        "from typing import Optional, Dict, Any\n\n"
        "class StreamPipelineContext(BaseModel):\n"
        "    token: str\n"
        ...
        "```\n\n"
        "```python\n"
        "# states/kafka_consumer_step.py\n"
        "from wpipe import step, to_obj\n"
        "from wkafka import WKafka\n"
        "from dto.stream_context import StreamPipelineContext\n\n"
        "@step(name='KafkaConsumerStep', version='v1.0', description='Consumes events from Kafka into WPipe Pipeline')\n"
        "class KafkaConsumerStep:\n"
        "    def __init__(self, bootstrap_servers='localhost:9092', topic='input_events'):\n"
        "        self.kafka = WKafka(bootstrap_servers=bootstrap_servers, dynamic_group_id=True)\n"
        "        self.topic = topic\n\n"
        "    @to_obj(StreamPipelineContext)\n"
        "    def __call__(self, context: StreamPipelineContext) -> StreamPipelineContext:\n"
        "        print(f'🎧 Listening on topic {self.topic} for context {context.token}...')\n"
        "        return context\n"
        "```\n\n"
        "```python\n"
        "# main.py\n"
        "from wpipe import Pipeline\n"
        "from states.kafka_consumer_step import KafkaConsumerStep\n"
        "from dto.stream_context import StreamPipelineContext\n\n"
        "if __name__ == '__main__':\n"
        "    pipeline = Pipeline(pipeline_name='wpipe_wkafka_stream', tracking_db='tracking.sqlite')\n"
        "    pipeline.set_steps([KafkaConsumerStep()])\n"
        "    result = pipeline.run({'token': 'tx_101', 'payload': {'action': 'process'}})\n"
