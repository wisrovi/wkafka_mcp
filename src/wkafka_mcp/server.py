...
def get_wkafka_architect_manual() -> str:
    ...
    return (
        ...
        "To build a structured WPipe pipeline worker microservice with WKafka, follow this exact structure:\n"
        "```python\n"
        "import sys\n"
        "import os\n"
        ...
        "from pipelines import horizontales_pipeline\n\n"
        ...
        "    )\n"
        "    logger.info(f'Task received for token: {context.token}')\n"
        "    try:\n"
        "        with ResourceMonitor('wpipe_project_monitor') as monitor:\n"
        "            with TaskTimer('wpipe_project_timer', timeout_seconds=3600) as timer:\n"
        "                @time_execution\n"
        "                def run():\n"
        "                    return horizontales_pipeline.run(context.model_dump())\n"
        "                run()\n"
        "        summary = monitor.get_summary()\n"
        "        logger.info(\n"
        "            f'Task completed in {timer.elapsed_seconds:.2f}s | '\n"
        "            f'Peak RAM: {summary.get(\"peak_ram_mb\", 0)} MB'\n"
        "        )\n"
        "    except ProcessError as e:\n"
        "        logger.error(f'Error in WPipe: {e}')\n"
        "    except Exception as e:\n"
        "        logger.opt(exception=True).error(f'Unexpected error: {e}')\n\n"
        "if __name__ == '__main__':\n"
        "    logger.info('Starting CNN Horizontales WPipe Microservice with WKafka...')\n"
        "    kafka.run_consumers(block=True)\n"
        "```"
    )
